...
2. Open `/messaging/whatsapp`: it shows the token, the number, the WhatsApp Business Account and where webhooks currently point. If the account shows as *unknown*, paste the **WhatsApp Business Account ID** (Meta → WhatsApp → API Setup, next to the phone number id) into the field on the page and save — no redeploy needed.
3. Press **Point this number's webhooks here** — an alternate callback set on the business phone number itself (Meta's most specific override; account-level is the fallback); the SAAS app's dashboard settings stay as they are. From then on the hotel receives every event and relays the SAAS admins' messages + all delivery receipts to SAAS (guest replies never reach the SAAS bot, which would otherwise answer them with "this is an automated number").
4. Press **Create / resubmit templates** — creates the missing guest templates in `en` + `ar` (bodies from `docs/message-content.md`) and resubmits rejected ones with the current text and category. Confirmation and pre-arrival are **Utility**; the post-stay message is **Marketing** (it carries the SAMA10 offer) and the dispatcher sends it only to guests with marketing consent. Status shows PENDING → APPROVED, usually within minutes. If Meta rejects one for `INCORRECT_CATEGORY`, the row shows Meta's verdict (e.g. *Meta says: MARKETING*): fix the wording in `src/lib/messaging/templates/whatsapp-bodies.ts` + `docs/message-content.md` (or the category in `MESSAGE_KIND_CATEGORY`, `src/lib/messaging/types.ts`), deploy, press the button again. An approved template whose text or category no longer matches the code gets a per-row **Resubmit** (it goes back to review and cannot be sent until approved again).
...
Meta Business Manager → WhatsApp Manager → Message templates → Create. Category **Utility** (post-stay: **Marketing**), one template per language (**English** and **Arabic**), names exactly:
...
**Marketing templates (`/templates`, super_admin).** Every template on the account with its live Meta status; *New template* builds one from scratch — header (text with one variable, or an image / video / PDF whose sample is uploaded to Meta from the editor), body with `{{1}}…` variables and sample values, footer, quick-reply / URL / phone buttons — validates it against Meta's limits and submits it (usually approved within minutes for MARKETING; check the status chips). Approved / rejected / paused templates can be edited (Meta reviews again) or deleted. To send one: *Campaigns → New campaign*, channel WhatsApp, message type *Approved template*, choose the template, say how each variable is filled (guest's name, first name, last room type, terms link, or fixed text), upload the header image if the template has one, check the preview, *Save & send*. Arabic guests get the Arabic variant when it is approved, everyone else English. Template campaigns go out at any time (no 24-hour window). Uploading a sample image for a template header needs the Meta **app id**: if `/messaging/whatsapp` shows the Meta app as *unknown*, enter it there (Meta for Developers → the app → the id at the top of the dashboard; for the shared SAAS app it is the "Mpire Notifications" app). Run migration `supabase/migrations/0012_campaign_templates.sql` if the database was not migrated by the session that shipped this (it adds `campaigns.wa_template_*` and the public `wa-media` bucket).
...
- **Words** — override, in English and Arabic, every headline and paragraph the home page shows (hero, welcome, rooms, experiences, facility captions, getting here, pay at the hotel, closing line), an optional announcement bar, and the intro paragraphs of the Rooms, The Peak and Contact pages. Grouped by page; empty = the built-in translation (shown as the placeholder).
