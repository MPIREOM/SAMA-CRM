...
type Form = { room_number: string; room_type_id: string; floor: string; status: "active" | "maintenance"; notes: string; sort_order: number };
...
export function RoomDialog({
  ...
  const [form, setForm] = useState<Form>({ room_number: "", room_type_id: types[0]?.id ?? "", floor: "", status: "active", notes: "", sort_order: 0 });
  ...
  useEffect(() => {
    ...
    if (room && room !== "new") {
      ...
      setForm({ room_number: "", room_type_id: types[0]?.id ?? "", floor: "", status: "active", notes: "", sort_order: 0 });
    }
  }, [room, types]);
