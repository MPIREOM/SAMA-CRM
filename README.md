...
- **Website control** — `/website` lets a super_admin replace every photo on the guest site (upload, library or default), override the home-page headlines and paragraphs and the page intros in both languages, show an announcement bar and switch optional home sections on or off — no deploy needed. Room photos are managed under `/rooms`, add-on photos under `/addons`.
